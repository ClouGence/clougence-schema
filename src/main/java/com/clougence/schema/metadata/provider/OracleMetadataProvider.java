...
import com.clougence.schema.metadata.domain.oracle.OracleIndex;
...
import static com.clougence.schema.metadata.domain.oracle.OracleSqlTypes.TIMESTAMP;
...
public class OracleMetadataProvider extends AbstractMetadataProvider implements MetaDataService {
    ...
    public Map<String, ColumnDef> getColumnMap(String catalog, String schema, String table) throws SQLException {
        String dbName = StringUtils.isNotBlank(catalog) ? catalog : schema;
        List<com.clougence.schema.metadata.domain.oracle.OracleColumn> columns = this.getColumns(dbName, table);
        if (columns != null) {
            return columns.stream().collect(Collectors.toMap(com.clougence.schema.metadata.domain.oracle.OracleColumn::getName, o -> o));
        } else {
    ...
    public List<com.clougence.schema.metadata.domain.oracle.OracleSchema> getSchemas() throws SQLException {
        try (Connection conn = this.connectSupplier.eGet()) {
    ...
    public com.clougence.schema.metadata.domain.oracle.OracleSchema getSchema(String schemaName) throws SQLException {
    ...
    public Map<String, List<com.clougence.schema.metadata.domain.oracle.OracleTable>> getTables(String[] schemaName) throws SQLException {
        ...
        try (Connection conn = this.connectSupplier.eGet()) {
            ...
            // group by schema
            return mapList.stream().map(this::convertTable).collect(Collectors.groupingBy(com.clougence.schema.metadata.domain.oracle.OracleTable::getSchema));
        }
    }

    public List<com.clougence.schema.metadata.domain.oracle.OracleTable> getAllTables() throws SQLException {
        String currentSchema = getCurrentSchema();
        return getAllTables(currentSchema);
    }

    public List<com.clougence.schema.metadata.domain.oracle.OracleTable> getAllTables(String schemaName) throws SQLException {
        if (StringUtils.isBlank(schemaName)) {
        ...
        //
        Map<String, List<com.clougence.schema.metadata.domain.oracle.OracleTable>> tableMap = this.getTables(new String[] { schemaName });
        if (tableMap == null || tableMap.isEmpty()) {
            return Collections.emptyList();
        } else {
            List<com.clougence.schema.metadata.domain.oracle.OracleTable> tableList = tableMap.get(schemaName);
    ...
    public List<com.clougence.schema.metadata.domain.oracle.OracleTable> findTable(String schemaName, String[] tableName) throws SQLException {
    ...
    public com.clougence.schema.metadata.domain.oracle.OracleTable getTable(String schemaName, String tableName) throws SQLException {
    ...
    public List<com.clougence.schema.metadata.domain.oracle.OracleColumn> getColumns(String schemaName, String tableName) throws SQLException {
        if (StringUtils.isBlank(tableName)) {
            return Collections.emptyList();
        }
        if (StringUtils.isBlank(schemaName)) {
    ...
    public List<com.clougence.schema.metadata.domain.oracle.OracleConstraint> getConstraint(String schemaName, String tableName) throws SQLException {
        if (StringUtils.isBlank(tableName)) {
            return Collections.emptyList();
        }
        if (StringUtils.isBlank(schemaName)) {
    ...
    public List<com.clougence.schema.metadata.domain.oracle.OracleConstraint> getConstraint(String schemaName, String tableName, com.clougence.schema.metadata.domain.oracle.OracleConstraintType... cType) throws SQLException {
        List<com.clougence.schema.metadata.domain.oracle.OracleConstraint> constraintList = getConstraint(schemaName, tableName);
        if (constraintList == null || constraintList.isEmpty()) {
            return constraintList;
        }
        return constraintList.stream().filter(mySqlConstraint -> {
            for (com.clougence.schema.metadata.domain.oracle.OracleConstraintType constraintType : cType) {
    ...
    public com.clougence.schema.metadata.domain.oracle.OraclePrimaryKey getPrimaryKey(String schemaName, String tableName) throws SQLException {
        if (StringUtils.isBlank(tableName)) {
            return null;
        }
        if (StringUtils.isBlank(schemaName)) {
        ...
        try (Connection conn = this.connectSupplier.get()) {
            ...
            //
            Map<String, Optional<com.clougence.schema.metadata.domain.oracle.OraclePrimaryKey>> pkMap = mapList.stream().map(this::convertPrimaryKey).collect(Collectors.groupingBy(o -> {
            ...
            //
            Optional<com.clougence.schema.metadata.domain.oracle.OraclePrimaryKey> primaryKeyOptional = pkMap.values().stream().findFirst().orElse(Optional.empty());
            return primaryKeyOptional.orElse(null);
        }
    }

    public List<com.clougence.schema.metadata.domain.oracle.OracleUniqueKey> getUniqueKey(String schemaName, String tableName) throws SQLException {
        if (StringUtils.isBlank(tableName)) {
            return null;
        }
        if (StringUtils.isBlank(schemaName)) {
    ...
    public List<com.clougence.schema.metadata.domain.oracle.OracleForeignKey> getForeignKey(String schemaName, String tableName) throws SQLException {
        if (StringUtils.isBlank(tableName)) {
            return Collections.emptyList();
        }
        if (StringUtils.isBlank(schemaName)) {
    ...
    public List<com.clougence.schema.metadata.domain.oracle.OracleIndex> getIndexes(String schemaName, String tableName) throws SQLException {
        if (StringUtils.isBlank(tableName)) {
            return Collections.emptyList();
        }
        if (StringUtils.isBlank(schemaName)) {
    ...
    public List<com.clougence.schema.metadata.domain.oracle.OracleIndex> getIndexes(String schemaName, String tableName, com.clougence.schema.metadata.domain.oracle.OracleIndexType... indexTypes) throws SQLException {
        if (indexTypes == null || indexTypes.length == 0) {
            return Collections.emptyList();
        }
        List<com.clougence.schema.metadata.domain.oracle.OracleIndex> indexList = getIndexes(schemaName, tableName);
        if (indexList == null || indexList.isEmpty()) {
            return Collections.emptyList();
        }
        return indexList.stream().filter(indexItem -> {
            com.clougence.schema.metadata.domain.oracle.OracleIndexType indexTypeForItem = indexItem.getIndexType();
            for (com.clougence.schema.metadata.domain.oracle.OracleIndexType matchType : indexTypes) {
    ...
    public com.clougence.schema.metadata.domain.oracle.OracleIndex getIndexes(String schemaName, String tableName, String indexName) throws SQLException {
        List<com.clougence.schema.metadata.domain.oracle.OracleIndex> indexList = getIndexes(schemaName, tableName);
        if (indexList == null || indexList.isEmpty()) {
            return null;
        }
    ...
    protected com.clougence.schema.metadata.domain.oracle.OracleSchema convertSchema(Map<String, Object> recordMap) {
        com.clougence.schema.metadata.domain.oracle.OracleSchema schema = new com.clougence.schema.metadata.domain.oracle.OracleSchema();
        schema.setSchema(safeToString(recordMap.get("USERNAME")));
        schema.setStatus(com.clougence.schema.metadata.domain.oracle.OracleSchemaStatus.valueOfCode(safeToString(recordMap.get("ACCOUNT_STATUS"))));
        ...
        schema.setAuthenticationType(com.clougence.schema.metadata.domain.oracle.OracleSchemaAuthType.valueOfCode(safeToString(recordMap.get("AUTHENTICATION_TYPE"))));
        schema.setLastLogin(safeToDate(recordMap.get("LAST_LOGIN")));
        return schema;
    }

    protected com.clougence.schema.metadata.domain.oracle.OracleTable convertTable(Map<String, Object> recordMap) {
        com.clougence.schema.metadata.domain.oracle.OracleTable table = new com.clougence.schema.metadata.domain.oracle.OracleTable();
        ...
        table.setTableType(com.clougence.schema.metadata.domain.oracle.OracleTableType.valueOfCode(safeToString(recordMap.get("TABLE_TYPE"))));
        table.setMaterializedLog(safeToString(recordMap.get("LOG_TABLE")));
        table.setComment(safeToString(recordMap.get("COMMENTS")));
        return table;
    }

    protected com.clougence.schema.metadata.domain.oracle.OracleColumn convertColumn(Map<String, Object> recordMap, List<String> primaryKeyColumnList, List<String> uniqueKeyColumnList) {
        com.clougence.schema.metadata.domain.oracle.OracleColumn column = new com.clougence.schema.metadata.domain.oracle.OracleColumn();
    ...
    protected com.clougence.schema.metadata.domain.oracle.OracleConstraint convertConstraint(Map<String, Object> recordMap) {
        com.clougence.schema.metadata.domain.oracle.OracleConstraint constraint = new com.clougence.schema.metadata.domain.oracle.OracleConstraint();
        ...
        constraint.setConstraintType(com.clougence.schema.metadata.domain.oracle.OracleConstraintType.valueOfCode(constraintTypeString));
    ...
    protected com.clougence.schema.metadata.domain.oracle.OraclePrimaryKey convertPrimaryKey(Map<String, Object> recordMap) {
        com.clougence.schema.metadata.domain.oracle.OraclePrimaryKey primaryKey = new com.clougence.schema.metadata.domain.oracle.OraclePrimaryKey();
        primaryKey.setSchema(safeToString(recordMap.get("OWNER")));
        primaryKey.setName(safeToString(recordMap.get("CONSTRAINT_NAME")));
        primaryKey.setConstraintType(com.clougence.schema.metadata.domain.oracle.OracleConstraintType.PrimaryKey);
    ...
    protected com.clougence.schema.metadata.domain.oracle.OracleUniqueKey convertUniqueKey(Map<String, Object> recordMap) {
        com.clougence.schema.metadata.domain.oracle.OracleUniqueKey uniqueKey = new com.clougence.schema.metadata.domain.oracle.OracleUniqueKey();
        ...
        if ("U".equalsIgnoreCase(constraintType)) {
            uniqueKey.setConstraintType(com.clougence.schema.metadata.domain.oracle.OracleConstraintType.Unique);
        } else if ("P".equalsIgnoreCase(constraintType)) {
            uniqueKey.setConstraintType(com.clougence.schema.metadata.domain.oracle.OracleConstraintType.PrimaryKey);
        } else {
    ...
    protected com.clougence.schema.metadata.domain.oracle.OracleForeignKey convertForeignKey(Map<String, Object> recordMap) {
        com.clougence.schema.metadata.domain.oracle.OracleForeignKey foreignKey = new com.clougence.schema.metadata.domain.oracle.OracleForeignKey();
        foreignKey.setSchema(safeToString(recordMap.get("OWNER")));
        foreignKey.setName(safeToString(recordMap.get("CONSTRAINT_NAME")));
        foreignKey.setConstraintType(com.clougence.schema.metadata.domain.oracle.OracleConstraintType.ForeignKey);
        ...
        foreignKey.setDeleteRule(com.clougence.schema.metadata.domain.oracle.OracleForeignKeyRule.valueOfCode(safeToString(recordMap.get("DELETE_RULE"))));
        //
    ...
    protected com.clougence.schema.metadata.domain.oracle.OracleIndex convertIndex(Map<String, Object> recordMap) {
        com.clougence.schema.metadata.domain.oracle.OracleIndex index = new OracleIndex();
        index.setSchema(safeToString(recordMap.get("OWNER")));
        index.setName(safeToString(recordMap.get("INDEX_NAME")));
        index.setIndexType(com.clougence.schema.metadata.domain.oracle.OracleIndexType.valueOfCode(safeToString(recordMap.get("INDEX_TYPE"))));
    ...
    protected JDBCType columnTypeMappingToJdbcType(SqlType sqlType, String columnType) {
        ...
        if (sqlType instanceof com.clougence.schema.metadata.domain.oracle.OracleSqlTypes) {
            switch ((com.clougence.schema.metadata.domain.oracle.OracleSqlTypes) sqlType) {
                case TIMESTAMP:
    ...
    protected com.clougence.schema.metadata.domain.oracle.OracleSqlTypes safeToOracleTypes(Object obj) {
        if (obj == null) {
            return null;
        }
        return com.clougence.schema.metadata.domain.oracle.OracleSqlTypes.toOracleType(obj.toString());
    }
}

...
import com.clougence.schema.metadata.domain.postgres.driver.PgServerVersion;
...
public class PostgresMetadataProvider extends AbstractMetadataProvider implements MetaDataService {
    ...
    public Map<String, ColumnDef> getColumnMap(String catalog, String schema, String table) throws SQLException {
        String dbName = StringUtils.isNotBlank(catalog) ? catalog : schema;
        List<com.clougence.schema.metadata.domain.postgres.PostgresColumn> columns = this.getColumns(dbName, table);
        if (columns != null) {
            return columns.stream().collect(Collectors.toMap(com.clougence.schema.metadata.domain.postgres.PostgresColumn::getName, o -> o));
        } else {
    ...
    public List<com.clougence.schema.metadata.domain.postgres.PostgresSchema> getSchemas() throws SQLException {
        try (Connection conn = this.connectSupplier.eGet()) {
    ...
    public com.clougence.schema.metadata.domain.postgres.PostgresSchema getSchema(String schemaName) throws SQLException {
    ...
    public Map<String, List<com.clougence.schema.metadata.domain.postgres.PostgresTable>> getTables(String[] schemaName) throws SQLException {
        ...
        try (Connection conn = this.connectSupplier.eGet()) {
            ...
            return mapList.stream().map(this::convertTable).collect(Collectors.groupingBy(com.clougence.schema.metadata.domain.postgres.PostgresTable::getSchema));
        }
    }

    public List<com.clougence.schema.metadata.domain.postgres.PostgresTable> getAllTables() throws SQLException {
        String currentSchema = getCurrentSchema();
        return getAllTables(currentSchema);
    }

    public List<com.clougence.schema.metadata.domain.postgres.PostgresTable> getAllTables(String schemaName) throws SQLException {
        ...
        //
        Map<String, List<com.clougence.schema.metadata.domain.postgres.PostgresTable>> tableMap = this.getTables(new String[] { schemaName });
        if (tableMap == null || tableMap.isEmpty()) {
            return Collections.emptyList();
        } else {
            List<com.clougence.schema.metadata.domain.postgres.PostgresTable> tableList = tableMap.get(schemaName);
    ...
    public List<com.clougence.schema.metadata.domain.postgres.PostgresTable> findTable(String schemaName, String[] tableName) throws SQLException {
    ...
    public com.clougence.schema.metadata.domain.postgres.PostgresTable getTable(String schemaName, String tableName) throws SQLException {
    ...
    public List<com.clougence.schema.metadata.domain.postgres.PostgresColumn> getColumns(String schemaName, String tableName) throws SQLException {
        if (StringUtils.isBlank(tableName)) {
            return Collections.emptyList();
        }
        if (StringUtils.isBlank(schemaName)) {
    ...
    public List<com.clougence.schema.metadata.domain.postgres.PostgresConstraint> getConstraint(String schemaName, String tableName) throws SQLException {
        if (StringUtils.isBlank(tableName)) {
            return Collections.emptyList();
        }
        if (StringUtils.isBlank(schemaName)) {
    ...
    public List<com.clougence.schema.metadata.domain.postgres.PostgresConstraint> getConstraint(String schemaName, String tableName, com.clougence.schema.metadata.domain.postgres.PostgresConstraintType... cType) throws SQLException {
        List<com.clougence.schema.metadata.domain.postgres.PostgresConstraint> constraintList = getConstraint(schemaName, tableName);
        if (constraintList == null || constraintList.isEmpty()) {
            return constraintList;
        }
        return constraintList.stream().filter(mySqlConstraint -> {
            for (com.clougence.schema.metadata.domain.postgres.PostgresConstraintType constraintType : cType) {
    ...
    public com.clougence.schema.metadata.domain.postgres.PostgresPrimaryKey getPrimaryKey(String schemaName, String tableName) throws SQLException {
        if (StringUtils.isBlank(tableName)) {
            return null;
        }
        if (StringUtils.isBlank(schemaName)) {
        ...
        try (Connection conn = this.connectSupplier.eGet()) {
            ...
            //
            Map<String, Optional<com.clougence.schema.metadata.domain.postgres.PostgresPrimaryKey>> pkMap = mapList.stream().sorted((o1, o2) -> {
                // sort by key_seq
            ...
            //
            Optional<com.clougence.schema.metadata.domain.postgres.PostgresPrimaryKey> primaryKeyOptional = pkMap.values().stream().findFirst().orElse(Optional.empty());
            return primaryKeyOptional.orElse(null);
        }
    }

    public List<com.clougence.schema.metadata.domain.postgres.PostgresUniqueKey> getUniqueKey(String schemaName, String tableName) throws SQLException {
        if (StringUtils.isBlank(tableName)) {
            return Collections.emptyList();
        }
        if (StringUtils.isBlank(schemaName)) {
    ...
    public List<com.clougence.schema.metadata.domain.postgres.PostgresForeignKey> getForeignKey(String schemaName, String tableName) throws SQLException {
        if (StringUtils.isBlank(tableName)) {
            return Collections.emptyList();
        }
        if (StringUtils.isBlank(schemaName)) {
    ...
    public List<com.clougence.schema.metadata.domain.postgres.PostgresIndex> getIndexes(String schemaName, String tableName, com.clougence.schema.metadata.domain.postgres.PostgresIndexType[] indexTypes) throws SQLException {
        if (indexTypes == null || indexTypes.length == 0) {
            return Collections.emptyList();
        }
        List<com.clougence.schema.metadata.domain.postgres.PostgresIndex> indexList = getIndexes(schemaName, tableName);
        if (indexList == null || indexList.isEmpty()) {
            return Collections.emptyList();
        }
        return indexList.stream().filter(indexItem -> {
            for (com.clougence.schema.metadata.domain.postgres.PostgresIndexType matchType : indexTypes) {
    ...
    public com.clougence.schema.metadata.domain.postgres.PostgresIndex getIndexes(String schemaName, String tableName, String indexName) throws SQLException {
        List<com.clougence.schema.metadata.domain.postgres.PostgresIndex> indexList = getIndexes(schemaName, tableName);
        if (indexList == null || indexList.isEmpty()) {
            return null;
        }
    ...
    public List<com.clougence.schema.metadata.domain.postgres.PostgresIndex> getIndexes(String schemaName, String tableName) throws SQLException {
        if (StringUtils.isBlank(tableName)) {
            return Collections.emptyList();
        }
        if (StringUtils.isBlank(schemaName)) {
    ...
    protected com.clougence.schema.metadata.domain.postgres.PostgresSchema convertSchema(Map<String, Object> recordMap) {
        com.clougence.schema.metadata.domain.postgres.PostgresSchema schema = new PostgresSchema();
        schema.setSchema(safeToString(recordMap.get("schema_name")));
        schema.setOwner(safeToString(recordMap.get("schema_owner")));
        return schema;
    }

    protected com.clougence.schema.metadata.domain.postgres.PostgresTable convertTable(Map<String, Object> recordMap) {
        com.clougence.schema.metadata.domain.postgres.PostgresTable table = new com.clougence.schema.metadata.domain.postgres.PostgresTable();
        table.setSchema(safeToString(recordMap.get("table_schema")));
        table.setTable(safeToString(recordMap.get("table_name")));
        table.setTableType(com.clougence.schema.metadata.domain.postgres.PostgresTableType.valueOfCode(safeToString(recordMap.get("table_type"))));
    ...
    protected com.clougence.schema.metadata.domain.postgres.PostgresColumn convertColumn(Map<String, Object> recordMap, List<String> primaryKeyColumnList, List<String> uniqueKeyColumnList, long serverVersionNumber) {
        com.clougence.schema.metadata.domain.postgres.PostgresColumn column = new com.clougence.schema.metadata.domain.postgres.PostgresColumn();
    ...
    protected com.clougence.schema.metadata.domain.postgres.PostgresConstraint convertConstraint(Map<String, Object> recordMap) {
        com.clougence.schema.metadata.domain.postgres.PostgresConstraint constraint = new com.clougence.schema.metadata.domain.postgres.PostgresConstraint();
        ...
        constraint.setConstraintType(com.clougence.schema.metadata.domain.postgres.PostgresConstraintType.valueOfCode(constraintTypeString));
        return constraint;
    }

    protected com.clougence.schema.metadata.domain.postgres.PostgresPrimaryKey convertPrimaryKey(Map<String, Object> recordMap) {
        com.clougence.schema.metadata.domain.postgres.PostgresPrimaryKey primaryKey = new com.clougence.schema.metadata.domain.postgres.PostgresPrimaryKey();
        primaryKey.setConstraintType(com.clougence.schema.metadata.domain.postgres.PostgresConstraintType.PrimaryKey);
    ...
    protected com.clougence.schema.metadata.domain.postgres.PostgresUniqueKey convertUniqueKey(Map<String, Object> recordMap) {
        com.clougence.schema.metadata.domain.postgres.PostgresUniqueKey uniqueKey = new com.clougence.schema.metadata.domain.postgres.PostgresUniqueKey();
        uniqueKey.setSchema(safeToString(recordMap.get("table_schema")));
        uniqueKey.setName(safeToString(recordMap.get("index_name")));
        uniqueKey.setConstraintType(com.clougence.schema.metadata.domain.postgres.PostgresConstraintType.Unique);
        //
    ...
    protected com.clougence.schema.metadata.domain.postgres.PostgresForeignKey convertForeignKey(Map<String, Object> recordMap) {
        com.clougence.schema.metadata.domain.postgres.PostgresForeignKey foreignKey = new com.clougence.schema.metadata.domain.postgres.PostgresForeignKey();
        ...
        foreignKey.setConstraintType(com.clougence.schema.metadata.domain.postgres.PostgresConstraintType.ForeignKey);
        //
        foreignKey.setUpdateRule(com.clougence.schema.metadata.domain.postgres.PostgresForeignKeyRule.valueOfCode(safeToString(recordMap.get("update_rule"))));
        foreignKey.setDeleteRule(com.clougence.schema.metadata.domain.postgres.PostgresForeignKeyRule.valueOfCode(safeToString(recordMap.get("delete_rule"))));
        foreignKey.setMatchOption(com.clougence.schema.metadata.domain.postgres.PostgresForeignMatchOption.valueOfCode(safeToString(recordMap.get("match_option"))));
        //
    ...
    protected com.clougence.schema.metadata.domain.postgres.PostgresIndex convertIndex(Map<String, Object> recordMap) {
        com.clougence.schema.metadata.domain.postgres.PostgresIndex pgIndex = new com.clougence.schema.metadata.domain.postgres.PostgresIndex();
        ...
        if (safeToBoolean(recordMap.get("non_unique"))) {
            pgIndex.setIndexType(com.clougence.schema.metadata.domain.postgres.PostgresIndexType.Normal);
        } else {
            pgIndex.setIndexType(com.clougence.schema.metadata.domain.postgres.PostgresIndexType.Unique);
        }
        //
        String columnName = safeToString(recordMap.get("column_name"));
        String ascOrDesc = safeToString(recordMap.get("asc_or_desc"));
    ...
    protected JDBCType columnTypeMappingToJdbcType(com.clougence.schema.metadata.domain.postgres.PostgresColumn column, Map<String, Object> recordMap) {
        if (recordMap == null) {
            return null;
        }
        ...
        //
        com.clougence.schema.metadata.domain.postgres.PostgresTypes sqlType = column.getSqlType();
    ...
    protected com.clougence.schema.metadata.domain.postgres.PostgresTypes safeToPostgresTypes(long serverVersionNumber, Map<String, Object> record) {
        if (record == null) {
            return null;
        }
        ...
        //
        com.clougence.schema.metadata.domain.postgres.PostgresTypes pgTypeEnum = com.clougence.schema.metadata.domain.postgres.PostgresTypes.valueOfTypeOid(typeOid.intValue());
        if (pgTypeEnum == null) {
            pgTypeEnum = com.clougence.schema.metadata.domain.postgres.PostgresTypes.valueOfTypeOid(typeOid);
            if (pgTypeEnum == null) {
                String name = typeName.toLowerCase();
                if (aliasMap.containsKey(name)) {
                    typeName = aliasMap.get(name);
                }
                pgTypeEnum = com.clougence.schema.metadata.domain.postgres.PostgresTypes.valueOfCode(typeName);
            }
        }
        //
        if (pgTypeEnum == null) {
        ...
        if (defaultValue != null && defaultValue.contains("nextval(")) {
            ...
            if (pgTypeEnum == com.clougence.schema.metadata.domain.postgres.PostgresTypes.INTEGER) {
                pgTypeEnum = com.clougence.schema.metadata.domain.postgres.PostgresTypes.SERIAL;
            } else if (pgTypeEnum == com.clougence.schema.metadata.domain.postgres.PostgresTypes.BIGINT) {
                pgTypeEnum = com.clougence.schema.metadata.domain.postgres.PostgresTypes.BIGSERIAL;
            } else if (pgTypeEnum == com.clougence.schema.metadata.domain.postgres.PostgresTypes.SMALLINT && gte_9_2) {
                pgTypeEnum = com.clougence.schema.metadata.domain.postgres.PostgresTypes.SMALLSERIAL;
            }
        }
        //
        return pgTypeEnum;

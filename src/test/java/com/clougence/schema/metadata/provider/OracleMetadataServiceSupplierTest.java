...
public class OracleMetadataServiceSupplierTest extends AbstractMetadataServiceSupplierTest<OracleMetadataProvider> {
    ...
    public void getSchemasTest() throws SQLException {
        List<com.clougence.schema.metadata.domain.oracle.OracleSchema> schemas = this.repository.getSchemas();
        List<String> collect = schemas.stream().map(com.clougence.schema.metadata.domain.oracle.OracleSchema::getSchema).collect(Collectors.toList());
    ...
    public void getSchemaTest() throws SQLException {
        com.clougence.schema.metadata.domain.oracle.OracleSchema schema1 = this.repository.getSchema("abc");
        com.clougence.schema.metadata.domain.oracle.OracleSchema schema2 = this.repository.getSchema("SYSTEM");
        assert schema1 == null;
        assert schema2 != null;
    }
    ...
    public void getTables() throws SQLException {
        Map<String, List<com.clougence.schema.metadata.domain.oracle.OracleTable>> tableList = this.repository.getTables(new String[] { "SCOTT", "SYSTEM" });
        assert tableList.size() == 2;
        assert tableList.containsKey("SCOTT");
        assert tableList.containsKey("SYSTEM");
        List<String> tableForInformationSchema = tableList.get("SCOTT").stream().map(com.clougence.schema.metadata.domain.oracle.OracleTable::getTable).collect(Collectors.toList());
    ...
    public void findTables() throws SQLException {
        List<com.clougence.schema.metadata.domain.oracle.OracleTable> tableList = this.repository.findTable("SCOTT", new String[] { "PROC_TABLE_REF", "TB_USER_VIEW" });
        Map<String, com.clougence.schema.metadata.domain.oracle.OracleTable> tableMap = tableList.stream().collect(Collectors.toMap(com.clougence.schema.metadata.domain.oracle.OracleTable::getTable, o -> o));
        assert tableMap.size() == 2;
        assert tableMap.containsKey("PROC_TABLE_REF");
        assert tableMap.containsKey("TB_USER_VIEW");
        assert tableMap.get("PROC_TABLE_REF").getTableType() == com.clougence.schema.metadata.domain.oracle.OracleTableType.Table;
        assert tableMap.get("TB_USER_VIEW").getTableType() == com.clougence.schema.metadata.domain.oracle.OracleTableType.View;
    }

    @Test
    public void getTable() throws SQLException {
        com.clougence.schema.metadata.domain.oracle.OracleTable tableObj1 = this.repository.getTable("SCOTT", "PROC_TABLE_REF");
        com.clougence.schema.metadata.domain.oracle.OracleTable tableObj2 = this.repository.getTable("SCOTT", "TB_USER_VIEW");
        com.clougence.schema.metadata.domain.oracle.OracleTable tableObj3 = this.repository.getTable("SCOTT", "abc");
        com.clougence.schema.metadata.domain.oracle.OracleTable tableObj4 = this.repository.getTable("SCOTT", "TB_USER");
        assert tableObj1 != null;
        assert tableObj1.getTableType() == com.clougence.schema.metadata.domain.oracle.OracleTableType.Table;
        assert tableObj1.getMaterializedLog() == null;
        assert tableObj2 != null;
        assert tableObj2.getTableType() == com.clougence.schema.metadata.domain.oracle.OracleTableType.View;
        assert tableObj2.getMaterializedLog() == null;
        assert tableObj3 == null;
        assert tableObj4 != null;
        assert tableObj4.getTableType() == com.clougence.schema.metadata.domain.oracle.OracleTableType.Table;
        assert tableObj4.getMaterializedLog() != null;
    }

    @Test
    public void getColumns_1() throws SQLException {
        List<com.clougence.schema.metadata.domain.oracle.OracleColumn> columnList = this.repository.getColumns("SCOTT", "TB_ORACLE_TYPES");
        Map<String, com.clougence.schema.metadata.domain.oracle.OracleColumn> columnMap = columnList.stream().collect(Collectors.toMap(com.clougence.schema.metadata.domain.oracle.OracleColumn::getName, c -> c));
        //
        List<SqlType> collect1 = columnList.stream().map(com.clougence.schema.metadata.domain.oracle.OracleColumn::getSqlType).collect(Collectors.toList());
        List<JDBCType> collect2 = columnList.stream().map(com.clougence.schema.metadata.domain.oracle.OracleColumn::getJdbcType).filter(Objects::nonNull).collect(Collectors.toList());
    ...
    public void getColumns_2() throws SQLException {
        List<com.clougence.schema.metadata.domain.oracle.OracleColumn> columnList = this.repository.getColumns("SCOTT", "PROC_TABLE_REF");
        Map<String, com.clougence.schema.metadata.domain.oracle.OracleColumn> columnMap = columnList.stream().collect(Collectors.toMap(com.clougence.schema.metadata.domain.oracle.OracleColumn::getName, c -> c));
    ...
    public void getConstraint1() throws SQLException {
        List<com.clougence.schema.metadata.domain.oracle.OracleConstraint> columnList = this.repository.getConstraint("SCOTT", "PROC_TABLE_REF");
        Map<String, com.clougence.schema.metadata.domain.oracle.OracleConstraintType> typeMap = columnList.stream().collect(Collectors.toMap(com.clougence.schema.metadata.domain.oracle.OracleConstraint::getName, com.clougence.schema.metadata.domain.oracle.OracleConstraint::getConstraintType));
        Set<String> typeNameSet = columnList.stream().map(com.clougence.schema.metadata.domain.oracle.OracleConstraint::getName).collect(Collectors.toSet());
        Set<com.clougence.schema.metadata.domain.oracle.OracleConstraintType> typeEnumSet = columnList.stream().map(com.clougence.schema.metadata.domain.oracle.OracleConstraint::getConstraintType).collect(Collectors.toSet());
        //
        ...
        //
        assert typeMap.get("PROC_TABLE_REF_UK") == com.clougence.schema.metadata.domain.oracle.OracleConstraintType.Unique;
        assert typeMap.get("PTR") == com.clougence.schema.metadata.domain.oracle.OracleConstraintType.ForeignKey;
        assert typeEnumSet.contains(com.clougence.schema.metadata.domain.oracle.OracleConstraintType.PrimaryKey);
    }

    @Test
    public void getConstraint2() throws SQLException {
        List<com.clougence.schema.metadata.domain.oracle.OracleConstraint> columnList = this.repository.getConstraint("SCOTT", "PROC_TABLE_REF", com.clougence.schema.metadata.domain.oracle.OracleConstraintType.Unique);
        Map<String, com.clougence.schema.metadata.domain.oracle.OracleConstraintType> typeMap = columnList.stream().collect(Collectors.toMap(com.clougence.schema.metadata.domain.oracle.OracleConstraint::getName, com.clougence.schema.metadata.domain.oracle.OracleConstraint::getConstraintType));
        Set<com.clougence.schema.metadata.domain.oracle.OracleConstraintType> typeEnumSet = columnList.stream().map(com.clougence.schema.metadata.domain.oracle.OracleConstraint::getConstraintType).collect(Collectors.toSet());
        //
    ...
    public void getPrimaryKey1() throws SQLException {
        com.clougence.schema.metadata.domain.oracle.OraclePrimaryKey primaryKey = this.repository.getPrimaryKey("SCOTT", "PROC_TABLE_REF");
        assert primaryKey.getConstraintType() == com.clougence.schema.metadata.domain.oracle.OracleConstraintType.PrimaryKey;
    ...
    public void getPrimaryKey2() throws SQLException {
        com.clougence.schema.metadata.domain.oracle.OraclePrimaryKey primaryKey = this.repository.getPrimaryKey("SCOTT", "PROC_TABLE");
        assert primaryKey.getConstraintType() == com.clougence.schema.metadata.domain.oracle.OracleConstraintType.PrimaryKey;
        assert primaryKey.getName().startsWith("SYS_");
    ...
    public void getPrimaryKey3() throws SQLException {
        com.clougence.schema.metadata.domain.oracle.OracleTable table = this.repository.getTable("SCOTT", "T3");
        com.clougence.schema.metadata.domain.oracle.OraclePrimaryKey primaryKey = this.repository.getPrimaryKey("SCOTT", "T3");
        assert table != null;
    ...
    public void getUniqueKey() throws SQLException {
        com.clougence.schema.metadata.domain.oracle.OraclePrimaryKey primaryKey = this.repository.getPrimaryKey("SCOTT", "TB_USER");
        List<com.clougence.schema.metadata.domain.oracle.OracleUniqueKey> uniqueKeyList = this.repository.getUniqueKey("SCOTT", "TB_USER");
        Map<String, com.clougence.schema.metadata.domain.oracle.OracleUniqueKey> uniqueKeyMap = uniqueKeyList.stream().collect(Collectors.toMap(com.clougence.schema.metadata.domain.oracle.OracleUniqueKey::getName, u -> u));
        assert uniqueKeyMap.size() == 2;
        //
        assert uniqueKeyMap.containsKey(primaryKey.getName());
        assert uniqueKeyMap.get(primaryKey.getName()).getConstraintType() == com.clougence.schema.metadata.domain.oracle.OracleConstraintType.PrimaryKey;
        ...
        assert uniqueKeyMap.get("TB_USER_EMAIL_USERUUID_UINDEX").getConstraintType() == com.clougence.schema.metadata.domain.oracle.OracleConstraintType.Unique;
    ...
    public void getForeignKey() throws SQLException {
        List<com.clougence.schema.metadata.domain.oracle.OracleForeignKey> foreignKeyList1 = this.repository.getForeignKey("SCOTT", "TB_USER");
        assert foreignKeyList1.size() == 0;
        List<com.clougence.schema.metadata.domain.oracle.OracleForeignKey> foreignKeyList2 = this.repository.getForeignKey("SCOTT", "PROC_TABLE_REF");
        assert foreignKeyList2.size() == 1;
        com.clougence.schema.metadata.domain.oracle.OracleForeignKey foreignKey = foreignKeyList2.get(0);
        assert foreignKey.getConstraintType() == com.clougence.schema.metadata.domain.oracle.OracleConstraintType.ForeignKey;
    ...
    public void getIndexes1() throws SQLException {
        com.clougence.schema.metadata.domain.oracle.OraclePrimaryKey primaryKey = this.repository.getPrimaryKey("SCOTT", "TB_USER");
        List<com.clougence.schema.metadata.domain.oracle.OracleIndex> indexList = this.repository.getIndexes("SCOTT", "TB_USER");
        Map<String, com.clougence.schema.metadata.domain.oracle.OracleIndex> indexMap = indexList.stream().collect(Collectors.toMap(com.clougence.schema.metadata.domain.oracle.OracleIndex::getName, i -> i));
        assert indexMap.size() == 3;
        //
        Set<String> indexNameSet = indexList.stream().map(com.clougence.schema.metadata.domain.oracle.OracleIndex::getName).collect(Collectors.toSet());
        ...
        assert indexMap.get(primaryKey.getName()).getIndexType() == com.clougence.schema.metadata.domain.oracle.OracleIndexType.Normal;
        ...
        assert indexMap.get("TB_USER_EMAIL_USERUUID_UINDEX").getIndexType() == com.clougence.schema.metadata.domain.oracle.OracleIndexType.Normal;
        ...
        assert indexMap.get("NORMAL_INDEX_TB_USER").getIndexType() == com.clougence.schema.metadata.domain.oracle.OracleIndexType.Normal;
    ...
    public void getIndexes2() throws SQLException {
        com.clougence.schema.metadata.domain.oracle.OraclePrimaryKey primaryKey = this.repository.getPrimaryKey("SCOTT", "PROC_TABLE_REF");
        List<com.clougence.schema.metadata.domain.oracle.OracleIndex> indexList = this.repository.getIndexes("SCOTT", "PROC_TABLE_REF");
        Map<String, com.clougence.schema.metadata.domain.oracle.OracleIndex> indexMap = indexList.stream().collect(Collectors.toMap(com.clougence.schema.metadata.domain.oracle.OracleIndex::getName, i -> i));
        assert indexMap.size() == 3;
        ...
        assert indexMap.get("PROC_TABLE_REF_UK").getIndexType() == com.clougence.schema.metadata.domain.oracle.OracleIndexType.Normal;
        ...
        assert indexMap.get("PROC_TABLE_REF_INDEX").getIndexType() == com.clougence.schema.metadata.domain.oracle.OracleIndexType.Normal;
    ...
    public void getIndexes3() throws SQLException {
        List<com.clougence.schema.metadata.domain.oracle.OracleIndex> indexList = this.repository.getIndexes("SCOTT", "PROC_TABLE_REF", com.clougence.schema.metadata.domain.oracle.OracleIndexType.Bitmap);
        Map<String, com.clougence.schema.metadata.domain.oracle.OracleIndex> indexMap = indexList.stream().collect(Collectors.toMap(com.clougence.schema.metadata.domain.oracle.OracleIndex::getName, i -> i));
    ...
    public void getIndexes4() throws SQLException {
        ...
        assert index.getIndexType() == com.clougence.schema.metadata.domain.oracle.OracleIndexType.Normal;
    }
}